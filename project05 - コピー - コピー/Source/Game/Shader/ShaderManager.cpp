...
void ShaderManager::LoadAll()
{
	for (int i = 0; i < ShaderManager::Shader_Max; ++i)
	{
		...
		if (FAILED(D3DXCreateEffectFromFile(CRendererDirectX::GetDevice(), shaderInfo_[i].fileName, NULL, NULL, 0, NULL, &effect_[i], NULL)))
		{
			MessageBox(NULL, "シェーダー読み込み失敗", "Error", MB_YESNO);
		}
	}
}
