...
bool XModel::LoadXFile(XModel::XMODEL modelType)
{
	...
	if (FAILED(hr))
	{
		MessageBox(NULL, "Xmodel読み込み失敗", "警告", MB_OK | MB_ICONHAND);
		return false;
	}
	...
	for (unsigned long i = 0; i < this->xmodel[modelType].nMaterialNum; i++)
	{
		...
		{
			hr = D3DXCreateTextureFromFile(pDevice, pMaterials[i].pTextureFilename, &this->xmodel[modelType].pMeshTextures[i]);
			if (FAILED(hr)) {
				this->xmodel[modelType].pMeshTextures[i] = NULL;
				MessageBox(NULL, "Xmodelテクスチャ読み込みに失敗しました。", "警告", MB_OK | MB_ICONHAND);
				D3DXCreateTextureFromFile(pDevice, "resource/Xmodel/default.png", &this->xmodel[modelType].pMeshTextures[i]);
			}
		}
		else
		{
